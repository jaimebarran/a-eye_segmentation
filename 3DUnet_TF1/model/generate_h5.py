...
def build_h5_dataset(data_path, target_path):
    ...
    d_imgshape = (64, new_shape[0], new_shape[1], new_shape[2],2)
    d_labelshape = (64, new_shape[0], new_shape[1], new_shape[2])

    d_imgshape_r1 = (64, new_shape[1], new_shape[0], new_shape[2],2)
    d_labelshape_r1 = (64, new_shape[1], new_shape[0], new_shape[2])

    d_imgshape_r2 = (64, new_shape[2], new_shape[1], new_shape[0],2)
    d_labelshape_r2 = (64, new_shape[2], new_shape[1], new_shape[0])

    d_imgshape_r3 = (64, new_shape[0], new_shape[2], new_shape[1],2)
    d_labelshape_r3 = (64, new_shape[0], new_shape[2], new_shape[1])
    ...
    for dirpath, dirnames, filenames in os.walk(data_path):
        for filename in [f for f in filenames if (("T1" in f)and('.nii'in f)) ]:
            f_T1= os.path.join(dirpath,filename)
            img_T1 = nib.load(f_T1)
            f_T2 = f_T1.replace("T1", "T2")
            img_T2 = nib.load(f_T2)
            f_l = f_T2.replace("T2", "GT") 
            img_l = nib.load(f_l)

            # combine T1 and T2 as 2 channels
            inputs_tmp_T1 = img_T1.get_data()
            inputs_tmp_T2 = img_T2.get_data()
            inputs = np.stack((inputs_tmp_T1,inputs_tmp_T2),axis=3)
