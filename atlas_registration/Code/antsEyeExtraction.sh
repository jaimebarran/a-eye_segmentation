...
Example:
  ...
  bash $0 -d 3 -a t1.nii.gz -e brainWithSkullTemplate.nii.gz -g eye_rois.nii.gz -o output
...
Required arguments:
     ...
     -g:  Eye ROIs                              Segmentation of the right eye.
...
echoParameters() {
    ...
    Using antsBrainExtraction with the following arguments:
      ...
      extraction eye ROIs     = ${EXTRACTION_EYE_ROIS}
...
if [[ $# -lt 3 ]] ; then
  ...
  while getopts "a:c:d:e:f:g:h:k:m:o:q:r:s:u:z:" OPT
    do
      case $OPT in
          d) #dimensions
       DIMENSION=$OPTARG
...
EXTRACTION_WARPED_OUTPUT=${BRAIN_EXTRACTION_OUTPUT}Normalized.${OUTPUT_SUFFIX}
...
EXTRACTION_EYE_ROIS_WARPED=${EXTRACTION_WARP_OUTPUT_PREFIX}EyeROIs.${OUTPUT_SUFFIX}
...
TMP_FILES=( $EXTRACTION_MASK_PRIOR_WARPED $EXTRACTION_WARP $EXTRACTION_WARPED_OUTPUT $EXTRACTION_INVERSE_WARP $EXTRACTION_TMP $EXTRACTION_GM $EXTRACTION_CSF $EXTRACTION_SEGMENTATION $EXTRACTION_INITIAL_AFFINE $EXTRACTION_INITIAL_AFFINE_MOVING $EXTRACTION_INITIAL_AFFINE_FIXED $EXTRACTION_LAPLACIAN $EXTRACTION_TEMPLATE_LAPLACIAN $EXTRACTION_WM )
    ...
    if [[ ! -f ${EXTRACTION_INVERSE_WARP} ]];
      then
        ...
        ## Step 1 ##
          ...
          basecall="${ANTS} -d ${DIMENSION} -u 1 -w [ 0.025,0.975 ] -o [${EXTRACTION_WARP_OUTPUT_PREFIX},${EXTRACTION_WARPED_OUTPUT}] -r ${EXTRACTION_INITIAL_AFFINE} -z 1 --float ${USE_FLOAT_PRECISION} --verbose 1"
          stage1="-m MI[ ${EXTRACTION_TEMPLATE},${N4_CORRECTED_IMAGES[0]},${ANTS_LINEAR_METRIC_PARAMS} ] -c ${ANTS_LINEAR_CONVERGENCE} -t Rigid[ 0.1 ] -f 8x4x2x1 -s 4x2x1x0 --masks [NULL,NULL]"
          stage2="-m MI[ ${EXTRACTION_TEMPLATE},${N4_CORRECTED_IMAGES[0]},${ANTS_LINEAR_METRIC_PARAMS} ] -c ${ANTS_LINEAR_CONVERGENCE} -t Affine[ 0.1 ] -f 8x4x2x1 -s 4x2x1x0 --masks [NULL,NULL]"
          stage3="-m CC[ ${EXTRACTION_TEMPLATE},${N4_CORRECTED_IMAGES[0]},0.5,4 ] -m CC[ ${EXTRACTION_TEMPLATE_LAPLACIAN},${EXTRACTION_LAPLACIAN},0.5,4 ] -c [ 50x10x0,1e-9,15 ] -t ${ANTS_TRANSFORMATION} -f 4x2x1 -s 2x1x0 --masks [${EXTRACTION_REGISTRATION_MASK},NULL]"
        ...
        exe_brain_extraction_2="${WARP} -d ${DIMENSION} -i ${EXTRACTION_EYE_ROIS} -o ${EXTRACTION_EYE_ROIS_WARPED} -r ${ANATOMICAL_IMAGES[0]} -n MultiLabel -t [ ${EXTRACTION_GENERIC_AFFINE},1 ] -t ${EXTRACTION_INVERSE_WARP} --float ${USE_FLOAT_PRECISION} --verbose 1"
        logCmd $exe_brain_extraction_2

    echo
    echo "--------------------------------------------------------------------------------------"
    echo " Done with eye extraction:  $(( time_elapsed_brain_extraction / 3600 ))h $(( time_elapsed_brain_extraction %3600 / 60 ))m $(( time_elapsed_brain_extraction % 60 ))s"
    echo "--------------------------------------------------------------------------------------"
    echo
  fi
