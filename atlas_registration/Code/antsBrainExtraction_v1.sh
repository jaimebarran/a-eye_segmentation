...
EXTRACTION_WARPED_OUTPUT_PREFIX=${BRAIN_EXTRACTION_OUTPUT}Normalized.${OUTPUT_SUFFIX}
...
TMP_FILES=( $EXTRACTION_MASK_PRIOR_WARPED $EXTRACTION_WARP $EXTRACTION_WARPED_OUTPUT_PREFIX $EXTRACTION_INVERSE_WARP $EXTRACTION_TMP $EXTRACTION_GM $EXTRACTION_CSF $EXTRACTION_SEGMENTATION $EXTRACTION_INITIAL_AFFINE $EXTRACTION_INITIAL_AFFINE_MOVING $EXTRACTION_INITIAL_AFFINE_FIXED $EXTRACTION_LAPLACIAN $EXTRACTION_TEMPLATE_LAPLACIAN $EXTRACTION_WM )
    ...
    if [[ ! -f ${EXTRACTION_INVERSE_WARP} ]];
      then
        ...
        ## Step 1 ##
          ...
          basecall="${ANTS} -d ${DIMENSION} -u 1 -w [ 0.025,0.975 ] -o [${EXTRACTION_WARP_OUTPUT_PREFIX},${EXTRACTION_WARPED_OUTPUT_PREFIX}] -r ${EXTRACTION_INITIAL_AFFINE} -z 1 --float ${USE_FLOAT_PRECISION} --verbose 1"
          if [[ -f ${EXTRACTION_REGISTRATION_MASK} ]];
            then
